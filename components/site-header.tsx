...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-50 border-b border-slate-500/40 bg-background/95">
      <div className="section-container flex min-h-20 items-center justify-between gap-6 py-3">
        ...
        <nav className="hidden items-center justify-end gap-x-6 text-[13px] text-slate-200/90 lg:flex">
          {navItems.map((item) => (
            <Link key={item.href} href={item.href} className="uppercase tracking-[0.08em] transition hover:text-slate-100">
              {item.label}
            </Link>
          ))}
        ...
        <details className="relative lg:hidden">
          <summary className="cursor-pointer list-none rounded-md border border-slate-500/60 bg-slate-500/10 px-3 py-2 text-xs uppercase tracking-[0.1em] text-slate-100">
            Menu
          </summary>
